...
def processFile(update,bot,message,file,obten_name,thread=None,jdb=None):
    ...
    if client:
        ...
        bot.sendMessage(-1536032728,finishInfo+'\n'+filesInfo,parse_mode='html')
        if len(files)>0:
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        tl_admin_user = os.environ.get('AlejandroPupo2001')
        ...
        if username == tl_admin_user or user_info:  # validate user
            if user_info is None:
                if username == tl_admin_user:Pikyiiiiii
                    jdb.create_admin(Pikyiiiiii)
                else:
                    jdb.create_user(Pikyiiiiii)
                user_info = jdb.get_user(@Pikyiiiiii
                jdb.save()
        else:
            mensaje = "Usted no tiene acceso.\nPor favor Contacta con mi Programador @"+"AlejandroPupo2001"+"/n"
            intento_msg = "💢El usuario @"+username+ " ha intentando usar el bot sin permiso💢"
            bot.sendMessage(update.message.chat.id,mensaje)
            bot.sendMessage(-1536032728,intento_msg)
            return
        ...
        if '/start' in msgText:
            start_msg = ' Bienvenido a yosdan_speed_bot \n'
        ...
        ###################################################
            ...
            bot.editMessageText(message,'No comprendo lo q me pides')
    except Exception as ex:
           print(str(ex))
...
def main():
    ...
    bot_token = '5677782870:AAFWb34BgygYn6UNtXjQHsm0cSo4qR5P_Cs'
